function player_charge() {
    ...
    if (charge && !auto_charge) {
        ...
        // Sonar cuando llega al nivel maximo con mejora de brazos
// Sonar cuando llega al nivel maximo
if (charge_level == charge_level_max) {
    if ((ARMS != "" || FULL != "") && !full_charge_sound_played) {
        audio_play(snd_full_check);
        full_charge_sound_played = true;
        // Crear aura si no existe
        if (!instance_exists(obj_charge_aura)) {
            var _aura = instance_create_depth(x, y, depth - 1, obj_charge_aura);
            _aura.owner = id;
        }
    }
} else {
